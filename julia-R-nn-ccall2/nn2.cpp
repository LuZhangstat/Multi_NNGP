...
{
  mod.method("mkNNIndxCB", &mkNNIndxCB);
}*/
...
void fastNN(int m, int n, jlcxx::ArrayRef<double> coords, int ui, double *u, int *sIndx, 
...
//extern "C" {
void mkNNIndxCB(int *nPtr, int *mPtr, jlcxx::ArrayRef<double> coords, 
                  jlcxx::ArrayRef<int> nnIndx, jlcxx::ArrayRef<double> nnDist, 
                  jlcxx::ArrayRef<int> nnIndxLU, int *nThreadsPtr){
/*int *nPtr, int *mPtr, jlcxx::ArrayRef<double> coords, 
 jlcxx::ArrayRef<int> nnIndx, jlcxx::ArrayRef<double> nnDist, 
 jlcxx::ArrayRef<int> nnIndxLU, int *nThreadsPtr, 
 int *nPtr, int *mPtr, double *coords, int *nnIndx, 
 double *nnDist, int *lnnIndxLU, int *nThreadsPtr*/
...
#ifdef _OPENMP
    printf("number of nThreads is %i", nThreads);
    omp_set_num_threads(nThreads);
#else
    if(nThreads > 1){
      printf("n.omp.threads > %i, but source not compiled with OpenMP support.", nThreads);
...
/*#JLCXX_MODULE define_julia_module(jlcxx::Module& nn2)
{
  nn2.method("mkNNIndxCB", &mkNNIndxCB);
}*/

//JULIA_CPP_MODULE_BEGIN(registry)
//  jlcxx::Module& nn2 = registry.create_module("CppNN");
//nn2.method("mkNNIndxCB", &mkNNIndxCB);
